 ...
 *
 * Five cards, each carrying a small working diagram of the thing it describes
 * rather than an icon. An icon says "search"; a list of numbers with a filter
 * under it says what searching here is actually like. They are drawn from the
 * app's own tokens and type, so they read as fragments of the product — which is
 * what they are — instead of as illustrations of it.
 *
 * Every fragment is `aria-hidden`. It is decoration for the sentence beneath it,
...
function Card({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'group grid content-start gap-5 rounded-[22px] border border-line-soft bg-surface-3 p-5 transition-[background-color,transform] duration-300',
    ...
    >
      ...
      <div>
        <h3 className="headline text-lg text-ink">{t(title)}</h3>
...
function Panel({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        'w-full overflow-hidden rounded-[14px] border border-line bg-surface shadow-md',
        className,
      )}
    >
      {children}
...
export function FeatureScene() {
  ...
  return (
    <Scene id="features" ground="bare" measure="full" className="bg-surface">
      ...
      <div className="mt-10 grid gap-3.5 sm:mt-14 lg:grid-cols-3">
        {/* ── Search ─────────────────────────────────── */}
        <Card
          delay={0.04}
        ...
        >
          <div className="grid w-full max-w-[18.75rem] gap-2.5" dir="ltr">
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-success/25 bg-success-soft px-3 py-3">
              ...
              <span className="font-mono text-[11px] tracking-[0.04em] text-success-ink">REGISTERED · 41ms</span>
